...
var purchase = {
    ...
    list_series: function (index) {
        purchase.details.products[index].series.map(function (value, index, array) {
            value.index = index;
        });
        tblSeries = $('#tblSeries').DataTable({
            ...
            autoWidth: false,
            destroy: true,
            data: purchase.details.products[index].series,
            columns: [
                {data: "index"},
                {data: "numfactpro"},
                {data: "serie"},
...
document.addEventListener('DOMContentLoaded', function (e) {
    ...
    fvSerie = FormValidation.formValidation(form, {
        ...
        .on('core.form.valid', function () {
            purchase.details.products[purchase.index].series.push({
                'serie': input_serie.val(),
                'numfactpro': input_numfactpro.val()
            });
            fvSerie.resetForm(true);
            input_serie.val('');
            input_numfactpro.val('').focus();
...
$(function () {
    ...
    input_numfactpro = $('input[name="numfactpro"]');
