...
from core.pos.models import PurchaseRequest, Company


class AuthorizePurchasePrintRequestView(LoginRequiredMixin, View):
    ...
    def get_height_ticket(self):
        sale = PurchaseRequest.objects.get(pk=self.kwargs['pk'])
        height = 120
        increment = sale.purchaserequestdetail_set.all().count() * 5.45
        height += increment
        return round(height)
    ...
    def get(self, request, *args, **kwargs):
        try:
            ...
            template = get_template('scm/authorizepurchase/print/order.html')
                #context['height'] = self.get_height_ticket()
            #else:
                #template = get_template('scm/purchaserequest/print/request.html')
