    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            if action == 'add':
                ...
                print('cat',product.category_id)
                product.marca_id = request.POST['marca']
                print('m',product.marca_id)
                product.codigo = request.POST['codigo']
                product.detalle = request.POST['detalle']
                product.stockmax = request.POST['stockmax']
                product.stockmin = request.POST['stockmin']
                product.modelo = request.POST['modelo']
                product.referencia = request.POST['referencia']
    ...
    def get(self, request, *args, **kwargs):
        try:
            headers = {
                ...
                'Precio de Venta': 20,
                'Stock': 10,
                #seag cambio de codigo a numero
                'Marca': 15,
                'Modelo': 15,
                'Referencia': 15,
                'Código': 15,
            ...
            for product in Product.objects.filter().order_by('id'):
                ...
                #seag
                worksheet.write(row, 6, product.marca.marca, row_format)
                worksheet.write(row, 7, product.modelo, row_format)
                worksheet.write(row, 8, product.referencia, row_format)
                worksheet.write(row, 9, product.codigo, row_format)
