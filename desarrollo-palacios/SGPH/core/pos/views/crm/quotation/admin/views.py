...
class QuotationSaleAdminListView(PermissionMixin, FormView):
    template_name = 'crm/quotation/admin/list.html'
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            if action == 'search':
                ...
                search = Sale.objects.filter(Q(estado='Cotización'))
    ...
    def get_context_data(self, **kwargs):
        ...
        context['create_url'] = reverse_lazy('quotationsale_admin_create')
        context['title'] = 'Listado de Cotizaciones Ventas'
        return context


class QuotationSaleAdminCreateView(PermissionMixin, CreateView):
    model = Sale
    template_name = 'crm/quotation/admin/create.html'
    form_class = SaleForm
    success_url = reverse_lazy('quotationsale_admin_list')
        ...
        try:
            if action == 'add':
                with transaction.atomic():
                    ...
                    sale.plazo_id = request.POST['plazo']
                    sale.sucursal_id = request.POST['sucursal']
                    print('pla',sale.sucursal_id)
                    sale.iva = float(Company.objects.first().iva) / 100
                    sale.dscto = float(request.POST['dscto']) 
                    sale.entrada = float(request.POST['entrada'])
                    sale.tval = float(request.POST['tval'])
                    print('tavl',sale.tval)
           ...
           #agreg condition plazo pricelist
                ...
                print('detprof',ventsjson)
                #detvent= json.loads(request.POST['det']) #agreg 'detsale':detvent

                template = get_template('crm/quotation/print/proforma.html')
    ...
    def get_context_data(self, **kwargs):
        ...
        context['title'] = 'Nuevo registro de una Solicitud de Venta'
        context['action'] = 'add'
        context['iva'] = Company.objects.first().get_iva()
        return context


#class QuotationSaleAdminDeleteView(PermissionMixin, DeleteView):
#    model = Sale
#    template_name = 'crm/quotation/admin/delete.html'
#    success_url = reverse_lazy('quotationsale_admin_list')
