...
class QuotationSalePrintVoucherView(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        try:
            ...
            if sale.type_voucher == 'ticket':
                template = get_template('crm/quotation/print/ticket.html')
                context['height'] = self.get_height_ticket()
            else:
                template = get_template('crm/quotation/print/invoice.html')
...
class QuotationSalePrintTitleView(LoginRequiredMixin, View):
    success_url = reverse_lazy('quotationsale_admin_list')
    ...
    def get(self, request, *args, **kwargs):
        try:
            ...
            context = {'sale': sale, 'company': Company.objects.first()}
            if sale.type_voucher == 'ticket':
                template = get_template('crm/quotation/print/title.html')
                context['height'] = self.get_height_ticket()
            else:
                template = get_template('crm/quotation/print/title.html')
            html_template = template.render(context).encode(encoding="UTF-8")
