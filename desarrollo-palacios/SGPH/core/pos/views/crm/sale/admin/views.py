    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            if action == 'search':
                ...
                #search = Sale.objects.filter(Q(estado='Pedido'))
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
           ...
           #agreg condition plazo pricelist
            ...
            elif action == 'search_rec':
                print('entra a recorrer c')
                data = []
                for i in Cards.objects.filter(id=request.POST['id']):
                    #print('entra a recorrer', str(i.desc))

                    #data.append({'id':i.id, 'name':i.desc})
                    data.append({'id':i.id,'porc':str(i.porc), 'name':i.name})
                    #data.append({'id':i.id,'desc':i.desc, 'name':i.name})
...
class SaleAdminUpdateView(PermissionMixin, UpdateView):
    model = Sale    
    template_name = 'crm/sale/admin/create.html'
    form_class = SaleForm
    success_url = reverse_lazy('sale_admin_list')
